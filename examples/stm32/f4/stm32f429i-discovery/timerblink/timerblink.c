...
int main(void)
{
  ...
  tim_setup();
    ...
    */  
    ...
    timer_set_oc_value(TIM1, TIM_OC1, 656.2);
    gpio_toggle(LGREENF_PORT, LGREENF);
    for (i = 0; i < 84000000; i++) {
			__asm__("nop");
		}

    timer_set_oc_value(TIM1, TIM_OC1, 721.82);
    gpio_toggle(LGREENF_PORT, LGREENF);
    for (i = 0; i < 42000000; i++) {
			__asm__("nop");
		}
    timer_set_oc_value(TIM1, TIM_OC1, 1312.4);
    gpio_toggle(LGREENF_PORT, LGREENF);
    for (i = 0; i < 126000000; i++) {
			__asm__("nop");
		}
    timer_set_oc_value(TIM1, TIM_OC1, 984.3);
    gpio_toggle(LGREENF_PORT, LGREENF);
    for (i = 0; i < 42000000; i++) {
			__asm__("nop");
		}
